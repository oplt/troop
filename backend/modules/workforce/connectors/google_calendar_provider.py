...
from backend.modules.workforce.connectors.builtins import build_google_calendar_manifest
...
class GoogleCalendarConnectorProvider:
    @property
    def manifest(self) -> ConnectorManifest:
        return build_google_calendar_manifest()

    async def authorize(
        self, db: AsyncSession, context: ConnectorAuthContext
    ) -> ConnectorAuthResult:
        try:
            result = await GoogleCalendarOAuthService(db).begin(
                context.owner_id,
    ...
    async def refresh(self, db: AsyncSession, installation_id: str) -> ConnectorAuthResult:
        installation = await load_installation(db, installation_id, provider_slug="google_calendar")
        adapter = GoogleCalendarAdapter(db, installation)
        try:
    ...
    async def health(self, db: AsyncSession, installation_id: str) -> ConnectorHealthResult:
        ...
        adapter = GoogleCalendarAdapter(db, installation)
        try:
            body = await adapter.request("GET", "/users/me/calendarList", params={"maxResults": 1})
            await db.commit()
            return ConnectorHealthResult(
                ok=True,
                status="healthy",
                reauth_required=installation.status == "reauthorization_required",
                details={"calendars": len(body.get("items") or [])},
            )
        except GoogleCalendarAPIError as exc:
            await db.commit()
            return ConnectorHealthResult(
    ...
    async def register_trigger(
        ...
        raise ValueError("Google Calendar triggers are not supported yet")
    ...
    async def normalize_event(self, raw_event: dict[str, Any], *, installation_id: str | None = None):
        ...
        raise ValueError("Google Calendar webhooks are not supported yet")
