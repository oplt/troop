...
from backend.modules.workforce.connectors.builtins import build_google_drive_manifest
...
class GoogleDriveConnectorProvider:
    @property
    def manifest(self) -> ConnectorManifest:
        return build_google_drive_manifest()

    async def authorize(
        self, db: AsyncSession, context: ConnectorAuthContext
    ) -> ConnectorAuthResult:
        try:
            result = await GoogleDriveOAuthService(db).begin(
                context.owner_id,
    ...
    async def refresh(self, db: AsyncSession, installation_id: str) -> ConnectorAuthResult:
        installation = await load_installation(db, installation_id, provider_slug="google_drive")
        adapter = GoogleDriveAdapter(db, installation)
        try:
    ...
    async def health(self, db: AsyncSession, installation_id: str) -> ConnectorHealthResult:
        ...
        adapter = GoogleDriveAdapter(db, installation)
        try:
            body = await adapter.request("GET", "/about", params={"fields": "user"})
            await db.commit()
            user = dict(body.get("user") or {}) if isinstance(body, dict) else {}
            return ConnectorHealthResult(
                ok=True,
                status="healthy",
                reauth_required=installation.status == "reauthorization_required",
                details={"email": user.get("emailAddress")},
            )
        except GoogleDriveAPIError as exc:
            await db.commit()
            return ConnectorHealthResult(
    ...
    async def register_trigger(
        ...
        raise ValueError("Google Drive triggers are not supported yet")
    ...
    async def normalize_event(self, raw_event: dict[str, Any], *, installation_id: str | None = None):
        ...
        raise ValueError("Google Drive webhooks are not supported yet")
