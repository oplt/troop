...
from backend.modules.workforce.connectors.builtins import build_linear_manifest
...
from backend.modules.workforce.integrations.linear import LinearAdapter, LinearAPIError, LinearOAuthService


class LinearConnectorProvider:
    @property
    def manifest(self) -> ConnectorManifest:
        return build_linear_manifest()

    async def authorize(
        self, db: AsyncSession, context: ConnectorAuthContext
    ) -> ConnectorAuthResult:
        try:
            result = await LinearOAuthService(db).begin(
                context.owner_id,
    ...
    async def refresh(self, db: AsyncSession, installation_id: str) -> ConnectorAuthResult:
        installation = await load_installation(db, installation_id, provider_slug="linear")
        adapter = LinearAdapter(db, installation)
        try:
    ...
    async def health(self, db: AsyncSession, installation_id: str) -> ConnectorHealthResult:
        ...
        adapter = LinearAdapter(db, installation)
        try:
            body = await adapter.graphql("query { viewer { id name email } }")
            viewer = dict(body.get("viewer") or {})
            await db.commit()
            return ConnectorHealthResult(
                ok=True,
                status="healthy",
                reauth_required=installation.status == "reauthorization_required",
                details={"email": viewer.get("email"), "name": viewer.get("name")},
            )
        except LinearAPIError as exc:
            await db.commit()
            return ConnectorHealthResult(
    ...
    async def register_trigger(
        ...
        raise ValueError("Linear triggers are not supported yet")
    ...
    async def normalize_event(self, raw_event: dict[str, Any], *, installation_id: str | None = None):
        ...
        raise ValueError("Linear webhooks are not supported yet")
