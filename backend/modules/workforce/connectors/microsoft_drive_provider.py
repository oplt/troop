"""Microsoft Drive (OneDrive/SharePoint) connector provider."""
...
from backend.modules.workforce.connectors.builtins import build_microsoft_drive_manifest
...
class MicrosoftDriveConnectorProvider:
    @property
    def manifest(self) -> ConnectorManifest:
        return build_microsoft_drive_manifest()

    async def authorize(
        self, db: AsyncSession, context: ConnectorAuthContext
    ) -> ConnectorAuthResult:
        try:
            result = await MicrosoftDriveOAuthService(db).begin(
                context.owner_id,
    ...
    async def refresh(self, db: AsyncSession, installation_id: str) -> ConnectorAuthResult:
        installation = await load_installation(db, installation_id, provider_slug="microsoft_drive")
        adapter = MicrosoftDriveAdapter(db, installation)
        try:
    ...
    async def health(self, db: AsyncSession, installation_id: str) -> ConnectorHealthResult:
        ...
        adapter = MicrosoftDriveAdapter(db, installation)
        try:
            body = await adapter.request("GET", "/me/drive", params={"$select": "id,driveType"})
            await db.commit()
            return ConnectorHealthResult(
                ok=True,
                status="healthy",
                reauth_required=installation.status == "reauthorization_required",
                details={"drive_type": body.get("driveType") if isinstance(body, dict) else None},
            )
        except MicrosoftDriveAPIError as exc:
            await db.commit()
            return ConnectorHealthResult(
    ...
    async def register_trigger(
        ...
        raise ValueError("Microsoft Drive triggers are not supported yet")
