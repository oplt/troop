...
from backend.modules.workforce.connectors.builtins import build_salesforce_manifest
...
class SalesforceConnectorProvider:
    @property
    def manifest(self) -> ConnectorManifest:
        return build_salesforce_manifest()

    async def authorize(
        self, db: AsyncSession, context: ConnectorAuthContext
    ) -> ConnectorAuthResult:
        try:
            result = await SalesforceOAuthService(db).begin(
                context.owner_id,
    ...
    async def refresh(self, db: AsyncSession, installation_id: str) -> ConnectorAuthResult:
        installation = await load_installation(db, installation_id, provider_slug="salesforce")
        adapter = SalesforceAdapter(db, installation)
        try:
    ...
    async def health(self, db: AsyncSession, installation_id: str) -> ConnectorHealthResult:
        ...
        adapter = SalesforceAdapter(db, installation)
        try:
            body = await adapter.request("GET", "/services/data/v59.0/limits")
            await db.commit()
            return ConnectorHealthResult(
                ok=True,
                status="healthy",
                reauth_required=installation.status == "reauthorization_required",
                details={"limits": list(body.keys())[:5]},
            )
        except SalesforceAPIError as exc:
            await db.commit()
            return ConnectorHealthResult(
    ...
    async def register_trigger(
        ...
        raise ValueError("Salesforce triggers are not supported yet")
    ...
    async def normalize_event(
        ...
        raise ValueError("Salesforce webhooks are not supported yet")
